...
class CrashUtils private constructor() {
    ...
    companion object {
        ...
        fun init(application: Application, crashDir: String): Boolean {
            ...
            if (mInitialized)
                return true
            if (Environment.MEDIA_MOUNTED == Environment.getExternalStorageState() && mApplication!!.get()?.getExternalCacheDir() != null)
                defaultDir = mApplication!!.get()?.getExternalCacheDir().toString() + FILE_SEP + "crash" + FILE_SEP
            else {
                defaultDir = mApplication!!.get()?.getCacheDir().toString() + FILE_SEP + "crash" + FILE_SEP
            }
